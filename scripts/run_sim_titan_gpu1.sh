#!/bin/bash
# ============================================================
# run_sim_titan_gpu1.sh  —  TitanX GPU 1: DexYCB Sim 抓取
# ============================================================
# 在 TitanX 服务器上执行:
#   CUDA_VISIBLE_DEVICES=1 bash scripts/run_sim_titan_gpu1.sh
# ============================================================

set -e

PROJ="/home/vision/Project/Affordance2Grasp"
ISAAC_SIM_PATH="/home/vision/isaacsim"
...
# 等待 grasps_candidate_dexycb 目录出现 (等本机同步)
MAX_WAIT=300
WAIT=0
while [ ! -d "$GRASP_DIR" ] || [ "$(ls $GRASP_DIR/*.hdf5 2>/dev/null | wc -l)" -eq 0 ]; do
    if [ $WAIT -ge $MAX_WAIT ]; then
        echo "❌ 超时等待 DexYCB HDF5 (${MAX_WAIT}s)，请先运行 sync_to_titan.sh"
        exit 1
    fi
    echo "  ⏳ 等待 DexYCB HDF5 同步... (${WAIT}s)"
    sleep 10
    WAIT=$((WAIT + 10))
done
...
echo ""
echo "============================================================"
echo "  GPU 1 DexYCB 完成: ✅$SUCCESS  ❌$FAILED  ⏭️$SKIPPED / $TOTAL"
echo "  结束时间: $(date)"
echo "  结果: $GT_DIR"
echo "============================================================"
