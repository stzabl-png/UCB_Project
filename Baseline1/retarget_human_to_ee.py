...
# subjects whose full GT (labels_*.npz / meta.yml / pose.npz) is present on disk
GT_SUBJECTS = ["20200928-subject-07", "20201002-subject-08",
...
def get_object_points(ycb_class_id, n_points, align_mode="auto"):
    """Return (n_points, 3) surface samples of ycb_dex_{id:02d}, in metric metres.

    align_mode:
      'none'  — points in the SAM3D mesh's own canonical frame (v1 behaviour: a constant
                per-object rotation offset vs the YCB-CAD frame that pose_y is defined in).
      'sam3d' — apply Baseline1/assets/sam3d_align/ycb_dex_NN.json (R_align: SAM3D→YCB-CAD
                canonical); errors loudly if the alignment file is missing.
      'auto'  — use 'sam3d' if the alignment file exists for this object, else 'none' (warn once).
    None if mesh missing. Cached per (object, n_points, align_mode)."""
    key = (ycb_class_id, n_points, align_mode)
    if key in _MESH_PTS_CACHE:
        return _MESH_PTS_CACHE[key]
    obj_dir = os.path.join(MESH_BASE, f"ycb_dex_{ycb_class_id:02d}")
    ...
    # bridge SAM3D canonical → YCB-CAD canonical so pose_y places it correctly
    if align_mode in ("sam3d", "auto"):
        align_path = os.path.join(ALIGN_DIR, f"ycb_dex_{ycb_class_id:02d}.json")
        if os.path.exists(align_path):
            R = np.asarray(json.load(open(align_path))["R_align_4x4"], dtype=np.float64)
            pts = (R[:3, :3] @ pts.T).T + R[:3, 3]
        elif align_mode == "sam3d":
            raise FileNotFoundError(
                f"--align-mode sam3d but no alignment for ycb_dex_{ycb_class_id:02d}; "
                f"run Baseline1/compute_sam3d_align.py first (needs DexYCB Step 3 done).")
        elif ycb_class_id not in _ALIGN_WARNED:
            print(f"  [align] no SAM3D↔YCB alignment for ycb_dex_{ycb_class_id:02d} yet — "
                  f"using SAM3D canonical frame (v1, has the per-object rotation offset)")
            _ALIGN_WARNED.add(ycb_class_id)
...
def build_episode(sess_dir, cam, meta, n_points, align_mode="auto"):
    ...
    obj_pts = get_object_points(ycb_class_id, n_points, align_mode)
    if obj_pts is None:
        return None, f"mesh ycb_dex_{ycb_class_id:02d} not found"
...
# ── main ──────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser(description="Baseline1 retarget — DexYCB route A")
        ...
        for sess in sessions:
            ...
            for cam in use_cams:
                ...
                if os.path.exists(out) and not args.redo:
                    continue
                ep, msg = build_episode(sess_dir, cam, meta, args.n_points, args.align_mode)
                tag = f"{subj}/{sess}/{cam}"
                if ep is None:
