...
|-------|-------|-------------|
| `grasp_point` | **mesh** | Virtual **pinch center** (two-finger midpoint semantics from UCB training) |
| `rotation` | **mesh** | Columns: `finger_open`, `y_body`, `approach` (col index 2 = approach axis) |
| `position_panda_hand` | **mesh** | Franka `panda_hand` TCP — **do not use for Dexmate IK** |
...
### Step R2 — Pinch → `R_ee` (one-time calibration)

Use virtual gripper **closed** posture (thumb + index intersection) aligned with `grasp_point`, same **approach** direction as column 2 of `rotation` (see `conventions.approach_column_index`).

Store fixed transform in `demo/phase2/calib/ee_retarget.yaml`:

```text
T_base_ee = T_base_pinch @ inv(T_ee_pinch)
```

Calibrate `T_ee_pinch` once per robot/hand setup (Phase 1 `pose_tuner` / FK vs pinch midpoint).
