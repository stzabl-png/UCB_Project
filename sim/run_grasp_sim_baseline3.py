...
def _build_plan_world_dict(scene, obj_pos_w, obj_quat):
    """Construct cuRobo world: table (cuboid) + ground (cuboid) + object (mesh if cached)."""
...
def _execute_grasp_curobo(scene, grasp_pos_w, grasp_quat_w, obj_pos_w, obj_quat):
    ...
    Pin policy: object is pinned across the cuRobo approach + grasp segments
    (planned to avoid the mesh, but finger-collision is auto-disabled inside
    the grasp segment → the pin keeps PhysX from spinning the body); released
    just before the gripper closes for the real lift physics test.
    """
    ...
    if not plan["success"]:
        ps = plan.get("phase_success", {})
        cprint(f"  ❌ cuRobo plan_grasp failed  phase={ps}", "red")
        cprint(f"    status: {plan['status'][:600]}", "red")
        return False, None

    a_q, g_q, l_q = plan["approach_qpos"], plan["grasp_qpos"], plan["lift_qpos"]
    cprint(f"  ✅ cuRobo plan_grasp: approach={len(a_q)} grasp={len(g_q)} lift={len(l_q)} wp  "
    ...
    # ---- approach: cuRobo avoided the mesh; still pin to neutralize any kinematic-jolt residue ----
    for qpos7 in a_q:
    ...
    # ---- grasp segment: pre-grasp -> grasp (cuRobo disabled finger collisions in
    #      its plan; PhysX will still react to gripper-finger insertion, so pin) ----
    for qpos7 in g_q:
        grip = franka.get_joint_positions()[7:9]
        full_q = np.concatenate([qpos7, grip])
        franka.set_joint_positions(full_q)
...
def _execute_grasp_legacy(scene, home_ee, grasp_pos_w, grasp_quat_w, obj_pos_w, obj_quat):
    ...
    for k in range(n_app):
        if not ok[k]:
            continue
        grip = franka.get_joint_positions()[7:9]
        franka.set_joint_positions(np.concatenate([qpos[k], grip]))
        for _ in range(2):
            world.step(render=True)
        _pin()
    _pin()
...
def main():
    ...
    for i, ep in enumerate(eps):
        ...
        # place the object at this episode's retarget G-pose
        sim_origin_W = np.array([OBJECT_XY[0] - obj_origin_G[0],
                                 OBJECT_XY[1] - obj_origin_G[1], TABLE_TOP_Z])
        obj_pos_w = obj_origin_G + sim_origin_W
        obj = scene["obj"]
        obj_pos_w = np.asarray(obj_pos_w, dtype=np.float64)
        scene["franka"].set_joint_positions(HOME_JOINTS)
        # CRITICAL: also reset the PD-controller target to HOME. If the previous
        # episode left it pointing at the lift-top qpos, PhysX articulation will
        # try to drag Franka back up — generating thousands of "Invalid PhysX
        # transform" warnings and eventually poisoning the parent process so that
        # all subsequent cuRobo plan_grasp subprocess calls crash.
        from omni.isaac.core.utils.types import ArticulationAction
        scene["franka"].apply_action(ArticulationAction(
            joint_positions=HOME_JOINTS, joint_velocities=np.zeros(9)))
        # Quick fix #1 (vs partner main): more settle steps so residual PhysX
        # state from previous ep is fully dissipated (was 40 → 150 to match
        # partner main).
        # Quick fix #2: explicit open_gripper here (already happens inside
        # _execute_grasp_curobo but with only 20 steps + pin) — gives the
        # gripper a clean opportunity to be open BEFORE plan starts.
        ...
        # Quick fix #3: sanity check — if previous ep poisoned the parent
        # (Franka joints went NaN/extreme, or object got flung 10^15 m away),
        # any further ep will plan_fail. Detect and exit early so the wrapper
        # script can spawn a fresh IsaacSim for the remaining eps.
        ...
        obj_sane = np.isfinite(obj_pos_now).all() and np.linalg.norm(np.asarray(obj_pos_now) - obj_pos_w) < 0.5
        if not (franka_sane and obj_sane):
            cprint(f"  ⚠️ pre-ep sanity check FAIL — franka_sane={franka_sane} "
                   f"obj_sane={obj_sane} qpos_max={np.nanmax(np.abs(qpos_now)):.2e} "
                   f"obj_pos={np.round(obj_pos_now,3).tolist()} (target {np.round(obj_pos_w,3).tolist()})",
                   "red")
            cprint(f"     parent process poisoned → EARLY-EXIT remaining {len(eps)-i} eps so wrapper can restart", "red")
            break

        # grasp pose = the human's contact pose (retarget action[-1]) → Franka convention, world
        grasp_pos_w = action[-1, :3] + sim_origin_W
        grasp_quat_w = retarget_to_franka_quat(action[-1, 3:7])

        cprint(f"\n[{i+1}/{len(eps)}] {name}", "yellow")
        video_begin()
        try:
            ok, rec_info = execute_grasp_b3(scene, home_ee, grasp_pos_w, grasp_quat_w,
                                            obj_pos_w, obj_quat_G)
        except Exception as e:
        ...
        st[1] += 1
        # rec_info is a dict {waypoints, grasp_onset_idx} on success; None on failure.
        ...
        if success:
            ...
            cprint(f"  ❌ grasp failed — not saved", "red")
        video_end(scene["world"], name.replace(".hdf5", ".mp4"),
