...
parser.add_argument("--titan-protocol", action="store_true", default=False,
                    help="ENABLE titan placement+PC protocol: place obj at identity quat "
                         "+ titan z_offset; live-sample PC from SAM3D × scale_factor "
                         "(NOT obj_quat_G from hdf5 + NOT hdf5 pc0).")
parser.add_argument("--titan-usd-dir",
                    default="/home/accelerator/UCB_Project_titan/output/obj_usd/oakink",
                    help="dir with titan SAM3D USDs + {obj_id}_meta.json (only used when --titan-protocol)")
parser.add_argument("--titan-mesh-root",
                    default="/home/accelerator/UCB_Project_titan/data_hub/meshes/SAM3DMesh/rotated_mesh",
                    help="root dir for prepare_metric_point_cloud (only used when --titan-protocol)")
...
# ★ titan PC sampler — loaded via importlib to bypass `model.*` namespace conflict ★
import importlib.util as _ilu
_TITAN_ROOT = "/home/accelerator/UCB_Project_titan"
def _load_titan_module(name, rel):
    p = os.path.join(_TITAN_ROOT, rel)
    sys.path.insert(0, _TITAN_ROOT)
    try:
        spec = _ilu.spec_from_file_location(name, p)
        m = _ilu.module_from_spec(spec); spec.loader.exec_module(m)
    finally:
        if sys.path and sys.path[0] == _TITAN_ROOT: sys.path.pop(0)
    return m
_titan_mp = _load_titan_module("titan_mesh_points", "model/pdm/mesh_points.py")
prepare_metric_point_cloud = _titan_mp.prepare_metric_point_cloud
...
def eval_one_episode(scene, ep_path, info):
    ...
    # ★ TITAN PROTOCOL OVERRIDE ★
    # Replace hdf5-recorded obj orientation + cached pc0 with:
    #   - identity quat (partner common-sense placement)
    #   - z_offset_m from titan SAM3D meta.json (= obj sits on table)
    #   - live-sampled PC from SAM3D × scale_factor (same source as titan USD)
    if args.titan_protocol and obj_id_attr:
        # Dataset-aware titan USD dir: oakink → titan_usd_dir; dexycb → swap to /ycb; unseen → swap to /unseen
        _ep_ds = str(h.attrs.get("dataset", "oakink")) if False else None  # set below from outer scope
        # Determine dataset from earlier read (cid + obj_id_attr in scope)
        ...
        if _is_unseen and "/oakink" in args.titan_usd_dir:
            ...
            _pc_dataset = "dexycb" if _is_dexycb else "oakink"
        ...
        if os.path.isfile(titan_meta_path):
            ...
            obj_quat_G = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)  # identity
            # Re-sample PC from titan SAM3D × scale_factor (canonical, identity quat → no rotation)
            pts, _, mesh_path = prepare_metric_point_cloud(
                obj_id_attr, mesh_root=args.titan_mesh_root,
                dataset=_pc_dataset, num_points=N_PC_POINTS, seed=0,
            )
            # pc in G-frame: with identity quat, PC = pts + obj_origin_G
            pc0 = (np.asarray(pts, dtype=np.float32) + np.array([0.0, 0.0, z_off_titan], dtype=np.float32))
            cprint(f"  ★ TITAN protocol: identity quat, z_off={z_off_titan:.4f}, "
                   f"pc_extent_cm={(pc0.max(0)-pc0.min(0))*100}  (mesh={os.path.basename(mesh_path)})", "magenta")
        else:
            cprint(f"  ⚠️ titan-protocol requested but {titan_meta_path} missing — falling back to gate3 defaults", "yellow")

    # ── Place object at FIXED OBJECT_XY + (titan or gate3) orientation ─────
...
def main():
    ...
    # ★ TITAN PROTOCOL: override USD path to titan's SAM3D-based USD (oakink or dexycb)
    if args.titan_protocol and oid:
        ...
        if os.path.isfile(titan_usd):
            ...
            cprint(f"  ⚠️ titan USD missing for {oid}: {titan_usd} — keeping gate3 CAD", "yellow")
