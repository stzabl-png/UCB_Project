...
# cuRobo's plan_grasp silently fails ("Goalset planning returned None") if the
# object mesh has too few faces (~ <500). All YCB textured_simple.obj are 15728
# faces so we set MAX above that — no decimation needed. Larger custom meshes
# get decimated; convex-hull fallback was REMOVED because it collapses the
# geometry to ~30 verts and triggers the same failure mode.
MIN_CUROBO_FACES = 500
MAX_CUROBO_FACES = 20000
...
def coarsen_mesh(vertices, faces, max_faces=MAX_CUROBO_FACES):
    ...
    If decimation fails or would produce a mesh below MIN_CUROBO_FACES
    (small meshes trigger a cuRobo planning failure mode), keep the original.
    """
    import trimesh
    ...
    try:
        # fast_simplification's `percent` is the REDUCTION fraction (0..1):
        # percent=0.96 → drop 96%, keep 4%. To target N faces from M, use 1-N/M.
        target_pct = max(0.10, min(0.99, 1.0 - max_faces / len(mesh.faces)))
        simplified = mesh.simplify_quadric_decimation(percent=target_pct)
        if len(simplified.faces) >= MIN_CUROBO_FACES:
            return simplified.vertices.astype(np.float64), simplified.faces.astype(np.int32)
    except Exception:
        pass
    # Fallback: keep the original (may be slow but correct). NEVER use convex_hull
    # — collapses to ~30 verts and breaks cuRobo plan_grasp.
    return mesh.vertices.astype(np.float64), mesh.faces.astype(np.int32)
...
def build_world_config_dict(
    ...
    if mesh_vertices is not None and mesh_faces is not None and mesh_pose_robot is not None:
        # cuRobo's Mesh.faces expects a FLAT list of vertex indices (not nested
        # [i,j,k] triplets) — see curobo/examples/getting_started/volumetric_mapping.py.
        # A nested list silently fails goalset IK with "returned None".
        import numpy as _np
        f_arr = _np.asarray(mesh_faces).reshape(-1).astype(_np.int64)
        v_arr = _np.asarray(mesh_vertices)
        cfg["mesh"] = {
            "grasp_object": {
                "vertices": v_arr.tolist(),
                "faces": f_arr.tolist(),
