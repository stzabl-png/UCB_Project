#!/usr/bin/env python3
"""
run_grasp_sim_pool.py — 候选池 sim 长驻 Isaac worker
==================================================
一次 SimulationApp 初始化，按 worker-chunk 顺序执行多个 (obj, candidate, yaw) task。
成功时 append 到 robot_gt/round_R/{obj}_robot_gt.hdf5。

用法 (由 batch_sim_candidates_pool.py 调用):
    $ISAAC_SIM_PATH/python.sh sim/run_grasp_sim_pool.py \\
...
parser.add_argument(
    "--worker-chunk", type=str, required=True,
    help="JSON: {chunk_id, round_idx, outdir, tasks: [...]}",
)
parser.add_argument("--outdir", type=str, default=None, help="grasp collect 根目录 (可含在 chunk 内)")
parser.add_argument("--headless", action="store_true", help="无头模式")
parser.add_argument("--object_scale", type=float, default=1.0, help="物体缩放 (默认 1.0)")
parser.add_argument(
    "--round-idx", type=int, default=None,
    help="round 索引 (可含在 chunk 内)",
)
...
def resolve_sim_z_yaw_deg(obj_id: str) -> float:
    """Pool worker: fixed yaw from current task."""
    if _WORKER_Z_YAW is not None:
        return float(_WORKER_Z_YAW)
    return 0.0
...
def _invalidate_rigid_physics_view(rigid) -> None:
    """Drop SingleRigidPrim tensor view before USD delete (avoid global sim view invalidate)."""
    if rigid is None:
        return
    try:
...
def _release_scene_rigid_object(scene: dict) -> None:
    """Remove scene['obj'] and release its PhysX handles."""
    old_obj = scene.pop("obj", None)
    if old_obj is None:
        return
...
def _remove_rigid_object_from_world(world, scene: dict | None = None) -> None:
    """
    换物体：先释放旧 rigid tensor view，再删 USD，最后 world.reset 重建 PhysX（Franka 保留）。
    """
...
def _apply_object_physics_materials(stage, obj):
    """碰撞 convexHull + 物体/指尖摩擦材料（与 setup_scene 一致）。"""
...
def _ensure_finger_friction_materials(stage):
...
def _spawn_rigid_object(
    ...
    *,
    ...
    """
    在已有 World 中加载物体 USD（不触碰 Franka / Table）。
    replace_existing=True 时先安全移除旧 rigid（方案 A 换物体）。
    Returns (obj, placement, mesh_info) or None.
    """
    usd_path = _find_obj_usd_path(obj_id)
    if usd_path is None:
    ...
    if replace_existing:
        _remove_rigid_object_from_world(world, scene)
    else:
        _delete_rigid_prims()
        for _ in range(20):
            world.step(render=RENDER_SIM)
...
def _update_scene_object(scene, obj, placement, mesh_info, obj_id: str):
    """更新 scene 内物体与 cuRobo mesh；强制下次规划重建 MotionGen。"""
    global _CUROBO_MG

    scene["obj"] = obj
    scene["obj_id"] = obj_id
    scene["object_placement"] = placement
    scene["sim_z_yaw_deg"] = placement["sim_z_yaw_deg"]
    if mesh_info is not None:
        scene["curobo_mesh_vertices"] = mesh_info["vertices"]
        scene["curobo_mesh_faces"] = mesh_info["faces"]
    else:
        scene.pop("curobo_mesh_vertices", None)
        scene.pop("curobo_mesh_faces", None)
    _CUROBO_MG = None


def _reset_franka_home(scene):
    """机械臂回 home + 开夹爪 + 物理稳定（同物体多 task 间复位）。"""
...
def swap_scene_object(scene, obj_id: str, object_scale: float, sim_z_yaw_deg: float) -> bool:
    """
    方案 A：保留 World + Franka，仅替换 RigidObject USD。
    候选/executed 仍在 object_mesh 系；sim Z-yaw 写入 world placement。
    """
    cprint(f"\n🔄 Swapping object → {obj_id} (yaw={sim_z_yaw_deg:.0f}°)...", "yellow")
    spawned = _spawn_rigid_object(
        scene["world"], obj_id, object_scale, sim_z_yaw_deg,
        scene=scene, replace_existing=True,
    )
    if spawned is None:
        return False
    obj, placement, mesh_info = spawned
    _update_scene_object(scene, obj, placement, mesh_info, obj_id)
    ...
    cprint(f"   ✅ Object swapped: {obj_id}", "green")
    return True
...
def setup_scene(obj_id, object_scale, sim_z_yaw_deg: float = 0.0):
    ...
    spawned = _spawn_rigid_object(world, obj_id, object_scale, sim_z_yaw_deg)
    if spawned is None:
        return None
    obj, placement, mesh_info = spawned
...
def _load_candidate_from_round_hdf5(h5_path: str, candidate_name: str) -> dict | None:
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        for i in range(n):
            ...
            if name != candidate_name:
                continue
            pose_pr = read_mesh_prerotation_hdf5_pose(ci, f)
            return {
                "name": name,
                "score": float(ci.attrs.get("score", 0.0)),
                "position": ci["position"][:],
                "rotation": ci["rotation"][:],
                "gripper_width": float(ci.attrs.get("gripper_width", 0.04)),
...
def _reset_scene_pose(scene, obj_id: str, object_scale: float, sim_z_yaw_deg: float):
    """Reset object pose + arm between tasks (same object, new yaw)."""
    global _CUROBO_MG
...
def _append_success_to_gt(
    ...
    *,
    ...
    write_mode = "a" if os.path.isfile(gt_path) else "w"
    with h5py.File(gt_path, write_mode) as rf:
        if write_mode == "w":
            rf.attrs["obj_id"] = obj_id
            rf.attrs["success"] = True
            rf.attrs["n_candidates_tried"] = 0
            rf.attrs["n_candidates_total"] = 0
            rf.attrs["n_successful"] = 0
            rf.attrs["object_scale"] = object_scale
            rf.attrs["robot_gt_schema_version"] = 2
            rf.attrs["executed_pose_frame"] = "object_mesh"
            rf.attrs["executed_ee_frame"] = "panda_hand"
            rf.attrs["sim_z_yaw_enabled"] = True
            rf.attrs["sim_z_yaw_deg"] = float(z_yaw_deg)
            rf.attrs["sim_z_yaw_round_idx"] = int(round_idx)
            rf.attrs["pool_sim"] = True
            if "successful_grasps" not in rf:
                rf.create_group("successful_grasps")

        sg = rf["successful_grasps"]
        existing = len(sg.keys())
        gi = sg.create_group(f"grasp_{existing}")
        ...
        gi.attrs["sim_z_yaw_deg"] = float(z_yaw_deg)
        gi.attrs["pool_candidate_key"] = candidate_key
        ...
        n_ok = existing + 1
        sg.attrs["count"] = n_ok
        rf.attrs["n_successful"] = n_ok
        rf.attrs["success"] = True


def _run_single_task(task: dict, outdir: str, round_idx: int, object_scale: float) -> dict:
    global _WORKER_Z_YAW, _CUROBO_MG

    obj_id = task["obj_id"]
    candidate_name = task["candidate_name"]
    z_yaw = float(task["z_yaw_deg"])
    candidate_key = task.get("candidate_key", candidate_name)

    tag = _round_tag(round_idx)
    h5_path = os.path.join(outdir, "candidates", tag, f"{obj_id}_grasp.hdf5")
    gt_path = os.path.join(outdir, "robot_gt", tag, f"{obj_id}_robot_gt.hdf5")
    ...
    if not os.path.isfile(h5_path):
        result["error"] = f"missing round grasp hdf5: {h5_path}"
        return result

    cand = _load_candidate_from_round_hdf5(h5_path, candidate_name)
    if cand is None:
        result["error"] = f"candidate {candidate_name} not in {h5_path}"
        return result

    _WORKER_Z_YAW = z_yaw
    cprint(f"\n{'='*50}", "yellow")
    cprint(
        f"  Task {task['task_id']}: {obj_id} / {candidate_name} / yaw={z_yaw:.0f}°",
        "yellow",
    )
    ...
    try:
        grasp_result = execute_grasp(
            ...
            object_scale,
            is_manual=False,
            mesh_prerotation_euler=cand.get("mesh_prerotation_euler"),
        )
        ...
        if success:
            cr = {
                ...
                "approach_type": cand.get("approach_type", ""),
                ...
                "mesh_prerotation": cand.get("mesh_prerotation"),
                "executed_at_close": grasp_result.get("executed_at_close"),
                "executed_post_lift": grasp_result.get("executed_post_lift"),
            }
            file_prerot = cand.get("mesh_prerotation")
            _append_success_to_gt(
                gt_path, obj_id, cr,
                object_scale=object_scale,
                round_idx=round_idx,
                z_yaw_deg=z_yaw,
                candidate_key=candidate_key,
                file_prerot=file_prerot,
            )
            cprint(f"  ✅ SUCCESS → appended GT", "green")
        else:
            cprint(f"  ❌ FAILED", "red")
    except Exception as e:
        result["error"] = str(e)
...
def worker_main():
    ...
    if not os.path.isfile(chunk_path):
        cprint(f"❌ worker-chunk not found: {chunk_path}", "red")
    ...
    outdir = args.outdir or chunk.get("outdir")
    round_idx = args.round_idx if args.round_idx is not None else chunk.get("round_idx")
    object_scale = float(chunk.get("object_scale", args.object_scale))
    tasks = chunk.get("tasks", [])

    if not outdir or round_idx is None:
        cprint("❌ outdir and round_idx required", "red")
        simulation_app.close()
        return

    cprint("=" * 60, "cyan")
    cprint("Pool Sim Worker", "cyan")
    cprint(f"  chunk: {chunk_path}  tasks={len(tasks)}", "cyan")
    cprint(f"  outdir: {outdir}  round: {round_idx}", "cyan")
    ...
    try:
        for task in tasks:
            ...
            if _WORKER_SCENE is None:
                ...
                swap_err = ""
                try:
                    swapped = swap_scene_object(
                        _WORKER_SCENE, obj_id, object_scale, z_yaw,
                    )
                except Exception as e:
                    swapped = False
                    swap_err = str(e)
                ...
                cprint(f"  → reset yaw {z_yaw:.0f}°", "yellow")
                _WORKER_Z_YAW = z_yaw
